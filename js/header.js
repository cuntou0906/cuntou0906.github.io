function search(searchIconUrl) {
    ...
    var searchData = userDefinedSearchData.custom ? userDefinedSearchData : {
        ...
        "data": [{
            ...
            name: "百度学术",
            img: "url(" + "/images/PageIcon/Search_Engines/BaiDuXueshuLogo.png" + ")",
            position: "0px 0px",
            size:"25px 25px",
            url: "https://xueshu.baidu.com/s?wd="
        }, {
            name: "谷歌学术",
            img: "url(" + "/images/PageIcon/Search_Engines/goolgeScholar.ico" + ")",
            position: "-40px 0px",
            size:"26px 26px",
            url: "https://scholar.google.com/scholar?hl=zh-CN&as_sdt=0%2C5&q="
        }, {
            name: "必应",
    ...
    for (var i = 0; i < searchData.data.length; i++) {
        ...
        if ((searchData.data[i].name=="谷歌学术")||(searchData.data[i].name=="百度学术")){
            $(".search-engine-list").append('<li><span style="background:' + searchData.data[i].img + (searchData.custom ? ' 0% 0% / cover no-repeat' : '') + 'no-repeat center; background-size: 70% 70%"/></span>' +
    ...
    $(".search-engine-list li").click(function () {
        ...
        if (searchData.custom) {
            ...
            if ((searchData.data[index].name=="谷歌学术")||(searchData.data[index].name=="百度学术")){
                // console.log(searchData.data[index].img)

                $(".search-icon").css("background", searchData.data[index].img +" no-repeat center");
                $(".search-icon").css("background-size", "70% 70%");

            }   
            else{
                // console.log(searchData.data[index].img)

                $(".search-icon").css("background", "url(/images/search_icon.png)");

                $(".search-icon").css("background-position", searchData.thisSearchIcon);
            }
        }
        searchData.thisSearch = searchData.data[index].url;
        $(".search-engine").css("display", "none");   // 隐藏搜索引擎
        localStorage.searchData = JSON.stringify(searchData)
    });
    ...
    if (searchData.custom) {
        ...
        // console.log(searchData.thisSearchIcon)
        $(".search-icon").css("background-position", searchData.thisSearchIcon);
...
function AddListenerOnMenu() {
    $(".subMenu").click(function(en){
        cur = $(this).parent()  // 当前对象

        par = $(this).parent()  // 父对象
        parpar = $(this).parent().parent()  // 父对象

        curIndex = $(this).index()  // 当前子对象的位置

        OneTitle = parpar[0].children[0].children[1].innerText  // 一级标题名称

        $('#'+ OneTitle + '>div >ul >li ').removeClass('active')
        $('#'+ OneTitle + ' >div >div ').removeClass('in active')


        $('#'+ OneTitle + '>div >ul >li:eq(' + curIndex +')').addClass('active')
        $('#'+ OneTitle + ' >div >div:eq(' + curIndex +')').addClass('in active')

        //  $("#MebuTab >li").removeClass("active")

        //  $(".main-content").find("ul:eq(parpar.index())")
      });

      $(".navLi ").click(function(en){
        subMenuIndex = $(this).index()
        MuneIndex = ($(this).parent().parent().parent().index()-1)/2
        $("#main-menu >li >ul> li").removeClass("active")

        $("#main-menu >li:eq(" + MuneIndex +") >ul> li:eq(" + subMenuIndex +")").addClass("active")
...
function switchNightMode() {
    ...
    //     // $('#MainContentDiv').addClass("main-content-Night")
    //     // $('#MainContentDiv').removeClass("main-content")
    //     // $('.MainContentTitleID').addClass("LibSpan-Night")
    //     // $('.MainContentTitleID').removeClass("LibSpan")
    //     // $('.nav>li>a >h3 >span').addClass("NavColor")
        
    //     //         // tab标签颜色设置
    //     // $('.tab .nav-tabs li.active a').css("background","rgb(77, 76, 76)")
    //     // $('.tab .nav-tabs li.active a i ').css("background","rgb(77, 76, 76)")

    //     // $(".nav.nav-tabs>li>a").hover(function(){
    //     //     $(".nav.nav-tabs>li>a").css("background-color","black"); 
    //     //     },function(){
    ...
    //     // $('#MainContentDiv').addClass("main-content")
    //     // $('#MainContentDiv').removeClass("main-content-Night")
    //     // $('.MainContentTitleID').removeClass("LibSpan-Night")
    //     // $('.MainContentTitleID').addClass("LibSpan")

    //     // tab标签颜色设置
    //     // $('.tab .nav-tabs li.active a').css("background","-webkit-gradient(linear, left top, left bottom, color-stop(0%,#f2e5db), color-stop(100%,#ffffff))")
    //     // $('.tab .nav-tabs li.active a i ').css("background","-webkit-gradient(linear, left top, left bottom, color-stop(0%,#f2e5db), color-stop(100%,#ffffff))")
    //     // $('.nav>li>a >h3 >span').removeClass("NavColor")
        
    //     // $(".nav.nav-tabs>li>a").hover(function(){
    //     //     $(".nav.nav-tabs>li>a").css("background-color","#ebf5f8"); 
    //     //     },function(){
    //     //         $("p").css("background-color","");
    //     //     });
